...
$sql ="DELETE FROM tblproduct WHERE id = '".$_GET["tewissen"]."'";
if ($mysqli->query($sql)){
    echo 'Record succesfully deleted from tblproduct.<br>';
} else {
    echo 'Error deleting record: ' . $mysqli->error;
}
$sql ="DELETE FROM tblwinkelwagen WHERE productid = '".$_GET["tewissen"]."'";
if ($mysqli->query($sql)){
    echo 'Record succesfully deleted from tblwinkelwagen.<br>';
} else {
    echo 'Error deleting record: ' . $mysqli->error;
}
$sql ="DELETE FROM menus WHERE product = '".$_GET["tewissen"]."'";
if ($mysqli->query($sql)){
    echo 'Record succesfully deleted from menus.';
} else {
    echo 'Error deleting record: ' . $mysqli->error;
}
$mysqli->close();
print "<br><a href='lijstproducten.php'><div class='link'>Back to product list</div></a>";
